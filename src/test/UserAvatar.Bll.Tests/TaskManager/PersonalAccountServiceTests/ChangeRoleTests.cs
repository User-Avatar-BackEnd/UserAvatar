...
public sealed class ChangeRoleTests
{
    ...
    public async Task ChangeRole_If_User_Is_Not_Exist_Returns_ResultCode_NotFound()
    {
        ...
        // Act
        var result =
            await personalAccountService.ChangeRoleAsync(It.IsAny<int>(), It.IsAny<string>(), It.IsAny<string>());
    ...
    public async Task ChangeRole_If_User_Is_UserToChangeRole_Returns_ResultCode_Forbidden()
    {
        ...
        _userStorage.Setup(x => x.GetByLoginAsync(It.IsAny<string>())).ReturnsAsync(new User { Id = userId });
